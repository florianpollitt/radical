...
int inline Internal::shrink_literal(int lit, int blevel, unsigned max_trail) {
  ...
  if (v.level < blevel) {
    ...
    LOG("literal %d on lower blevel %u < %u not removable/shrinkable", (lit),
        v.level, blevel);
    return -1;
  }
...
unsigned Internal::shrink_block(std::vector<int>::reverse_iterator &rbegin_lits,
  ...
  if (!failed) {
    ...
    while (!failed) {
      ...
      open += shrink_along_reason(uip, blevel, resolve_large_clauses, failed,
                                  max_trail2);
      assert(open >= 1);
    }
  ...
  if (failed)
    reset_shrinkable(),
        shrunken_block_no_uip(rbegin_lits, rend_block, block_minimized, uip0);
  else
    block_shrunken = shrunken_block_uip(uip, blevel, rbegin_lits, rend_block,
                                        minimized_start, uip0);
...
std::vector<int>::reverse_iterator Internal::minimize_and_shrink_block(
    ...
    unsigned &total_minimized, const int uip0)
  ...
  std::vector<int>::reverse_iterator rend_block;
  {
    ...
    do {
      ...
      if (!finished && (unsigned)vtab[idx].trail > max_trail)
        max_trail = vtab[idx].trail;
      ++open;
      LOG("testing if lit %i is on the same level (of lit: %i, global: %i)",
