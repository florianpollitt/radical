...
int Internal::second_literal_in_binary_clause(Eliminator &eliminator, Clause *c,
                                              int first) {
  assert(!c->garbage);
  int second = 0;
...
void Internal::mark_binary_literals(Eliminator &eliminator, int first) {
  ...
  for (const auto &c : os) {
    if (c->garbage)
      continue;
    const int second = second_literal_in_binary_clause(eliminator, c, first);
    if (!second)
      continue;
...
void Internal::find_equivalence(Eliminator &eliminator, int pivot) {
  ...
  for (const auto &c : occs(-pivot)) {
    ...
    const int second = second_literal_in_binary_clause(eliminator, c, -pivot);
    if (!second)
      continue;
    const int tmp = marked(second);
    if (tmp > 0) {
    ...
    for (const auto &e : ps) {
      if (e->garbage)
        continue;
      const int other = second_literal_in_binary_clause(eliminator, e, pivot);
      if (other == -second) {
        d = e;
        break;
      }
...
void Internal::find_and_gate(Eliminator &eliminator, int pivot) {
  ...
  for (const auto &c : occs(-pivot)) {
    ...
    for (const auto &d : occs(pivot)) {
      if (d->garbage)
        continue;
      const int other = second_literal_in_binary_clause(eliminator, d, pivot);
      if (!other)
        continue;
