...
void LratBuilder::collect_garbage_clauses() {
  ...
  LOG("LRAT BUILDER collecting %" PRIu64 " garbage clauses %.0f%%", num_garbage,
...
LratBuilder::LratBuilder(Internal *i)
    : internal(i), size_vars(0), vals(0), new_clause_taut(0),
      inconsistent(false), num_clauses(0), num_garbage(0), size_clauses(0),
      clauses(0), garbage(0), next_to_propagate(0), last_hash(0), last_id(0) {
...
void LratBuilder::add_clause(const char *type) {
  ...
  if (!size) {
    ...
    LOG("LRAT BUILDER added and checked falsified %s clause with id %" PRIu64,
        type, c->id);
    inconsistent = true;
    inconsistent_clause = c;
    ...
    if (!propagate()) {
      LOG("LRAT BUILDER inconsistent after adding %s clause and propagating",
          type);
