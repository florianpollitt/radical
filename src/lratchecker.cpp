...
void LratChecker::collect_garbage_clauses() {
  ...
  LOG("LRAT CHECKER collecting %" PRIu64 " garbage clauses %.0f%%", num_garbage,
...
bool LratChecker::check_resolution(vector<uint64_t> proof_chain) {
  ...
  for (int64_t lit = 1; lit < size_vars; lit++) {
    ...
    if (!ok) {
      LOG("LRAT CHECKER resolution failed, learned clause does not match on "
          "variable %" PRId64,
          lit);
      assert(false);
      return false;
...
bool LratChecker::check(vector<uint64_t> proof_chain) {
  ...
  for (auto &id : proof_chain) {
    ...
    if (c->used) {
      LOG("LRAT CHECKER LRAT failed. Id %" PRIu64 " was used multiple times",
          id);
    ...
    if (!unit) {
      ...
      // fails in decompose... I do not have a trace for instantiate but I am
      // sure it also fails this assert
