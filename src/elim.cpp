...
void Internal::elim_on_the_fly_self_subsumption(Eliminator &eliminator,
  ...
  // TODO: lrat. I think we need to know resolvent here. (lrat: resolvent +
  // antecendent) which should be c. probably nothing todo here...
...
bool Internal::resolve_clauses(Eliminator &eliminator, Clause *c, int pivot,
                               Clause *d, const bool propagate_eagerly) {
  ...
  for (auto &c : ps) {
    ...
    for (auto &d : ns) {
      ...
      if (!resolve_clauses(eliminator, c, pivot, d, false))
        continue;
      assert(!opts.lrat || opts.lratexternal || !lrat_chain.empty());
      Clause *r = new_resolved_irredundant_clause(); // TODO: clear lrat_chain
...
int Internal::elim_round(bool &completed) {
  ...
  PHASE("elim-round", stats.elimrounds,
        "eliminated %d variables %.0f%% in %" PRId64 " resolutions", eliminated,
...
void Internal::elim(bool update_limits) {
  ...
  if (unsat)
    ...
    LOG("elimination produced %zd units", (size_t)(trail.size() - propagated));
    if (!propagate()) {
