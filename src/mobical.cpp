...
#undef SIGNAL
  ...
  int64_t phases() {
    ...
    for (size_t i = 0; i < calls.size(); i++) {
      Call *c = calls[i];
      if (last && c->type != Call::VAL && c->type != Call::FAILED &&
  ...
  int64_t option_high_value(const char *, int64_t def, int64_t lo, int64_t hi);
...
private:
  ...
  struct Segment {
    size_t lo, hi;
    Segment(size_t l, size_t h) : lo(l), hi(h) { assert(0 < l), assert(l < h); }
  };
...
static int rounded_percent(double a, double b) { return 0.5 + percent(a, b); }
...
void Mobical::print_statistics() {
  ...
  if (Trace::ok > 0)
    terminal.green(true);
  cerr << Trace::ok << " ok " << rounded_percent(Trace::ok, Trace::generated)
       << "%";
  if (Trace::ok > 0)
    terminal.normal();
  cerr << ", ";
  ...
  if (shared) {
    ...
    cerr << "solved " << shared->solved << ": " << terr.blue_code()
         ...
         << terr.normal_code() << ", " << terr.magenta_code() << shared->unsat
         << " unsat " << rounded_percent(shared->unsat, shared->solved) << "%"
         << terr.normal_code() << ", " << shared->incremental << " incremental "
         << rounded_percent(shared->incremental, shared->solved) << "%" << endl
         << flush;
...
#define SIGNAL(SIG) old_##SIG##_handler = signal(SIG, child_signal_handler);
  SIGNALS
#undef SIGNAL
}
  ...
  do {
    ...
    for (size_t i = 0; i < size(); i++) {
      ...
      if (c->type == Call::SET) {
        ...
        if (!strcmp(c->name, "conflicts") || !strcmp(c->name, "decisions"))
          lo = -1, hi = INT_MAX;
...
void Trace::write_prefixed_seed(const char *prefix) {
  ostringstream ss;
  ss << prefix << '-' << setfill('0') << right << setw(20) << seed << ".trace"
     << flush;
  ...
  while ((ch = next()) != EOF) {
    ...
    if (enforce) {
      ...
      if (constraining && c->type != Call::CONSTRAIN && c->type != Call::RESET)
        error("'%s' after 'constrain %d' without 'constrain 0'", c->keyword(),
              constraining);
...
Mobical::Mobical()
    : mode(0), verbose(false),
      ...
      shrinking(false), running(false), time_limit(DEFAULT_TIME_LIMIT),
...
int Mobical::main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-h")) {
      ...
      if (!is_unsigned_str(argv[i]) || (space_limit = atol(argv[i])) < 0 ||
          space_limit > 1e9)
      ...
      if (seed_str)
        die("can not handle multiple seeds '%s' and '%s' (try '-h')", seed_str,
            argv[i]);
      if (input_path)
      ...
      if (strcmp(input_path, "-") && !strcmp(input_path, argv[i]))
        die("input '%s' and output '%s' are the same", input_path, argv[i]);
      output_path = argv[i];
    } else {
  ...
  if (mode & SEED) {
    assert(seed_str);
    prefix();
    cerr << "generating single trace from seed '" << seed_str << '\'' << endl;
  }
  if (mode & INPUT) {
    assert(input_path);
    prefix();
    cerr << "reading single trace from input '" << input_path << '\'' << endl;
  }
  if (mode & OUTPUT) {
    assert(output_path);
    prefix();
...
int main(int argc, char **argv) { return CaDiCaL::mobical.main(argc, argv); }
