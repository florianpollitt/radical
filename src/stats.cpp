...
void Stats::print(Internal *internal) {
  ...
  if (all || stats.blocked) {
    PRT("blocked:         %15" PRId64 "   %10.2f %%  of irredundant clauses",
        stats.blocked, percent(stats.blocked, stats.added.irredundant));
    PRT("  blockings:     %15" PRId64 "   %10.2f    internal", stats.blockings,
  ...
  if (all)
    PRT("compacts:        %15" PRId64 "   %10.2f    interval", stats.compacts,
  ...
  if (all || stats.conditioned) {
    PRT("conditioned:     %15" PRId64 "   %10.2f %%  of irredundant clauses",
  ...
  if (all || stats.cover.total) {
    PRT("covered:         %15" PRId64 "   %10.2f %%  of irredundant clauses",
  ...
  if (all || stats.all.eliminated) {
    ...
    PRT("  elimphases:    %15" PRId64 "   %10.2f    interval", stats.elimphases,
    ...
    PRT("  elimands:      %15" PRId64 "   %10.2f %%  and gates", stats.elimands,
    ...
    PRT("  elimxors:      %15" PRId64 "   %10.2f %%  xor gates", stats.elimxors,
  ...
  if (all || stats.all.fixed) {
    ...
    PRT("  probed:        %15" PRId64 "   %10.2f    per failed", stats.probed,
        relative(stats.probed, stats.failed));
    PRT("  hbrs:          %15" PRId64 "   %10.2f    per probed", stats.hbrs,
        relative(stats.hbrs, stats.probed));
    PRT("  hbrsizes:      %15" PRId64 "   %10.2f    per hbr", stats.hbrsizes,
        relative(stats.hbrsizes, stats.hbrs));
    PRT("  hbreds:        %15" PRId64 "   %10.2f %%  per hbr", stats.hbreds,
        percent(stats.hbreds, stats.hbrs));
    PRT("  hbrsubs:       %15" PRId64 "   %10.2f %%  per hbr", stats.hbrsubs,
        percent(stats.hbrsubs, stats.hbrs));
  }
  PRT("  units:         %15" PRId64 "   %10.2f    interval", stats.units,
      relative(stats.conflicts, stats.units));
  PRT("  binaries:      %15" PRId64 "   %10.2f    interval", stats.binaries,
  ...
  if (all || stats.conflicts) {
    ...
    PRT("  bumped:        %15" PRId64 "   %10.2f    per learned", stats.bumped,
  ...
  PRT("propagations:    %15" PRId64 "   %10.2f M  per second", propagations,
      relative(propagations / 1e6, t));
  ...
  if (all || stats.reduced) {
    ...
    PRT("  reductions:    %15" PRId64 "   %10.2f    interval", stats.reductions,
  ...
  if (all)
    PRT("rescored:        %15" PRId64 "   %10.2f    interval", stats.rescored,
        relative(stats.conflicts, stats.rescored));
  if (all || stats.restarts) {
    PRT("restarts:        %15" PRId64 "   %10.2f    interval", stats.restarts,
        relative(stats.conflicts, stats.restarts));
    PRT("  reused:        %15" PRId64 "   %10.2f %%  per restart", stats.reused,
  ...
  if (all || stats.subsumed) {
    ...
    PRT("  transreds:     %15" PRId64 "   %10.2f    interval", stats.transreds,
    ...
    PRT("  subirr:        %15" PRId64 "   %10.2f %%  of subsumed", stats.subirr,
        percent(stats.subirr, stats.subsumed));
    PRT("  subred:        %15" PRId64 "   %10.2f %%  of subsumed", stats.subred,
    ...
    PRT("  subchecks:     %15" PRId64 "   %10.2f    per tried", stats.subchecks,
  ...
  if (all || stats.htrs) {
    PRT("ternary:         %15" PRId64 "   %10.2f %%  of resolved", stats.htrs,
        percent(stats.htrs, stats.ternres));
    PRT("  phases:        %15" PRId64 "   %10.2f    interval", stats.ternary,
        relative(stats.conflicts, stats.ternary));
    PRT("  htr3:          %15" PRId64 "   %10.2f %%  ternary hyper ternres",
  ...
  if (all || vivified) {
    PRT("vivified:        %15" PRId64 "   %10.2f %%  of all clauses", vivified,
  ...
  if (all || stats.weakened) {
    ...
    PRT("  extensions:    %15" PRId64 "   %10.2f    interval", stats.extensions,
  ...
  LINE();
  MSG("%sseconds are measured in %s time for solving%s", tout.magenta_code(),
      internal->opts.realtime ? "real" : "process", tout.normal_code());
...
void Checker::print_stats() {
  ...
  MSG("assumptions:     %15" PRId64 "   %10.2f    per check", stats.assumptions,
  ...
  MSG("collisions:      %15" PRId64 "   %10.2f    per search", stats.collisions,
