...
int App::main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help") ||
      ...
      if (++i == argc)
        ...
        APPERR("multiple output file options '-o %s' and '-o %s'", output_path,
               argv[i]);
      else if (!force_writing && most_likely_existing_cnf_file(argv[i]))
        APPERR("output file '%s' most likely existing CNF (use '-f')", argv[i]);
      ...
      if (++i == argc)
        ...
        APPERR("multiple time limit '-t %s' and '-t %s'", time_limit_specified,
               argv[i]);
      ...
      set("--log");
             ...
             !strcmp(argv[i], "--force=1") || !strcmp(argv[i], "--force=true"))
      force_strict_parsing = 0, force_writing = true;
    else if (!strcmp(argv[i], "--strict") || !strcmp(argv[i], "--strict=1") ||
  ...
  if (preprocessing > 0 || localsearch > 0 ||
...
#endif
    if (conflict_limit >= 0) {
      solver->message("setting conflict limit to %d conflicts (due to '%s')",
    ...
    if (decision_limit >= 0) {
      solver->message("setting decision limit to %d decisions (due to '%s')",
  ...
  if (proof_specified) {
    if (!proof_path) {
      ...
      if (force_binary)
        solver->message("connected to terminal thus non-binary proof forced");
  ...
  solver->message("reading DIMACS file from %s'%s'%s%s", tout.green_code(),
                  dimacs_name, tout.normal_code(), help.c_str());
  bool incremental;
  ...
  if (incremental) {
    ...
    for (auto lit : cube_literals) {
      if (lit)
        ...
        if (!quiet) {
          if (reporting) {
            sprintf(buffer, "solving cube %zu / %zu %.0f%%", solved, cubes,
        ...
        if (!quiet) {
          ...
          sprintf(buffer,
                  ...
                  "%s",
                  tout.magenta_code(), time.delta, percent(solved, cubes),
                  time.sum, relative(1e3 * time.sum, solved),
                  tout.normal_code());
          if (reporting)
            solver->message();
    ...
    solver->message("%zu cubes solved %.0f%%", solved, percent(solved, cubes));
  ...
  if (extension_path) {
    ...
    solver->message("writing extension stack to %s'%s'%s", tout.green_code(),
                    extension_path, tout.normal_code());
