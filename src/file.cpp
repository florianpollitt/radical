...
FILE *File::open_file(Internal *internal, const char *path, const char *mode) {
  (void)internal;
...
FILE *File::open_pipe(Internal *internal, const char *fmt, const char *path,
                      const char *mode) {
#ifdef QUIET
  (void)internal;
#endif
...
FILE *File::write_pipe(Internal *internal, const char *fmt, const char *path) {
...
File *File::write(Internal *internal, const char *path) {
  ...
  if (has_suffix(path, ".xz"))
    ...
    file = write_pipe(internal, "7z a -an -txz -si -so > %s 2>/dev/null", path);
  else
